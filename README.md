...
## Description
This is a step-by-step demonstration on how to conduct a multilevel regression analysis using Python to investigate (1) the effects of Heat Vulnerability Index (HVI) and median age of a given neighborhood in New York City on the percentage of adults in that neighborhood with poor physical health, and (2) whether those effects differ by the five boroughs.<br>
Starting from explaining the need to use multilevel modeling for heretical data, this project aims to showcase a variety of tools offered by Python libraries for data extraction, manipulation,  visualization, and finally analysis using statistical models. <br> 
...
It is often observed that areas in close spatial proximity tend to share similar characteristics, thus forming "clusters/groups" and motivating researchers to conduct multilevel regressions to investigate both the within-group and between-group effects certain risk factors may have on a heat-related health outcome. <br>
In this study, the health outcome (the dependent variable Y) is percentage of adults with poor physical health residing in a given zip code, while the two predictor variables—HVI and median age for each zip code—have a hierarchical structure in that each zip code belongs to one of the five boroughs.  
...
For an example of how to conduct a multilevel regression using Python: [Advanced Statistics: Multilevel Regression](https://advstats.psychstat.org/python/multilevel/index.php)
...
This analysis will use three datasets that have been extracted and cleaned:
•	*df01*: Health Outcomes by Zip Code ( 'df01_health_by_Zip.csv')
Source: https://datacommons.org/place/geoId/3651000?category=Health <br>

•	*df02*: Averaged Heat Vulnerability (HVI) by Zip Code ('df02_HVI_Averages.csv')
Source: https://a816-dohbesp.nyc.gov/IndicatorPublic/data-features/hvi/) <br>

•	*df03*: Risk Factors by Zip Code ('Data_by_Zip.csv')
Source: https://simplemaps.com/city/new-york/zips/age-median<br>
...
These libraries: *numpy*,  *matplotlib.pyplot*, MaxNLocator  from *matplotlib.ticker*,  and *seaborn*  were used to create the following visualizations:
...
•	Multicolor scatter plots (one color for each borough) with a matching color regression line for each cluster of data points to show the relationship between *Percentage of Residents with Poor Physical Health* (health outcome) and *Median Age* (predictor) for each zip code. <br>
...
It is worth noting that both predictors  (average *HVI* and *Median Age*) were **cluster-mean centered**, which means the arithmetic mean of HVI and Median Age for each cluster (borough) were subtracted from each observation’s HVI and Median Age values, respectively in the corresponding borough. <br>
Mean-centering is a technique used in regression models when predictors do not have meaningful zero points. In our study, neither *Median Age* nor *HVI* have meaningful zero points because, without centering, the y-intercept would represent the average percentage of residents with poor physical health when the median age of those residents is 0 years old. Similarly, because heat vulnerability index ranges from 1 to 5, without centering HVI, the y-intercept would represent the average percentage of residents with poor physical health when HVI is zero, which is out of range for the index. <br>

**Step 4: Data Analysis**
Libraries used were: *statsmodels.api* and *statsmodels.formula.api*.
...
•	Finally, a multilevel model (mixed effects linear model) is specified. <br>
...
**[df02_HVI_Averages.csv]( https://a816-dohbesp.nyc.gov/IndicatorPublic/data-features/hvi/)** contains the following columns:<br>
•	*Zip*: five-digit zip codes used in NYC <br>
...
**HVI_Map_NYC.png** is a map of Heat Vulnerability Index for different neighborhoods in New York City. Source: [NYC Data Portal: Heat Vulnerability Index]( https://a816-dohbesp.nyc.gov/IndicatorPublic/data-features/hvi/) <br>
**Scatterplots_HVI_on_Poor_Health** is a juxtaposition of single-color vs. multicolor scatter plot where each color represents a borough. <br>
**Scatterplots_MedianAge_on_Poor_Health ** is a juxtaposition of single-color vs. multicolor scatter plot where each color represents a borough. <br>
**Linear_Regression_by_Cluster** shows the relationship between the health outcome (*Percentage of Residents with Poor Physical Health) and a single predictor (either *HVI* or *Median Age*) from simple linear (ordinary least squares) models. <br>
**Regression_Lines_Multilevel** shows the relationship between HVI (cluster-mean centered) and Percentage of Residents with Poor Physical Health while holding Median Age constant. Each borough is shown in different colors to highlight the varying intercepts and slopes of the different regression lines for different boroughs. <br>
